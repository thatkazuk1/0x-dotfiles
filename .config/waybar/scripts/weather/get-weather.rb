...
module Config
  ...
  class << self
    ...
    def load_user_config
      cfg_path = File.join(__dir__, 'weather-settings.jsonc')
...
module Icons
  class << self
    def init(icon_type)
      ...
      all_ui_icons = Utils.load_json(File.join(__dir__, 'ui-icons.json'))
    ...
    def load_icon_map(script_path)
      data = Utils.load_json(File.join(script_path, 'weather-icons.json'))
...
module ForecastData
  class << self
    ...
    def fetch_openmeteo_forecast(lat, lon, unit_c, daily_number_of_days = 16)
      ...
      if response.is_a?(Net::HTTPServerError) || response.code == '429'
        raise Net::HTTPError.new('Transient HTTP #{response.code}', response)
      end
...
# --- Error Handling ---
rescue Net::HTTPError, SocketError, Timeout::Error, Errno::ENETUNREACH, Errno::EHOSTUNREACH => e
  retries ||= 0
  retries += 1
  if retries <= 6
    sleep [2 ** retries, 30].min   # 2, 4, 8, 16, 30, 30 — ~90s total
    retry
  end
  puts JSON.generate(text: '…', tooltip: "network/server error after #{retries} retries: #{e.message}")
